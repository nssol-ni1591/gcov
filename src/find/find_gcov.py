...
#	PATTERN1 = re.compile("(\w*)\t([\w\/]*)\/bin\/([\w\/]*)\t([\S ]*)\t([\S ]*)\t([\S ]*)\t([\S ]*)$")
	...
	PATTERN4 = re.compile("^Lines executed:([\d\.]+)% of ([\d]+)")
...
#				self.debug("make_projlist: rec=[%s]" % rec)
				...
				if result2 :
					unit = result2.group(1)
					category = [result2.group(2), result2.group(3), result2.group(4), result2.group(5)]
					component = result2.group(6)
				else :
					self.warning("mapping file format. (rec=[%s])" % rec)
					continue

				self.debug("make_projlist: add unit=[%s] category=[%s] component=[%s]" % (unit, " ".join(category), component))
...
#			self.debug("path_to_category: unit=[%s] category=[%s] cached" % (self.CACHE_UNIT, " ".joun(" ", self.CAHCE_CATEGORY)))
		...
		for unit in self.CATELIST.keys() :
			pos = path.find(unit + "/")
#			self.debug("path_to_category: unit=[%s] pos=[%d]" % (unit, pos))
			if pos > 0 :
				self.CACHE_CATEGORY = self.CATELIST[unit]
				self.CACHE_UNIT = unit
				self.debug("path_to_category: return unit=[%s] category=[%s]" % (self.CACHE_UNIT, " ".join(self.CACHE_CATEGORY)))
				return self.CACHE_CATEGORY
	...
	def gcov_csv(self, path) :
		...
		for rec in iter(gcov.stdout.readline, b'') :
			rec = rec.rstrip("\n")
...
#			self.debug("gcov_csv: rec=[%s]" % rec)
			...
			if result3 :
				...
				self.debug("gcov_csv: function=[%s] coverage=[%s] lines=[%s]" % (function, result4.group(1), result4.group(2)))
	...
	def find_gcno(self, dir) :
		find = subprocess.Popen([ "/cygdrive/c/cygwin/bin/find", dir, "-name", "*.gcno" ], stdout=subprocess.PIPE)
