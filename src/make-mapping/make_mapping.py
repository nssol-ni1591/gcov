...
#					self.debug("exists_parent_unit: skip unit=[%s] component=[%s] d=[%s]" % (unit, component, d))
	...
	def find_unit(self, dir, component) :
		...
		paths = []
		find = subprocess.Popen(["find", dir, "-name", component, "-type", "f"], stdout = subprocess.PIPE)
...
#				self.debug("make_complist: rec=[%s]" % rec)
				...
				category = []
				component = ""
				result2 = self.PATTERN2.match(reec)
				if result2 :
				...
				if not component or componernt == "0" or component == "-" :
					continue
				if self.skip_component(topdir, component) :
					continue
